...
`c001apk` 是一个基于 Kotlin/Android 的 Coolapk 学习型第三方客户端。项目通过 Coolapk 的公开页面/API 形态复现首页、动态、评论、用户、应用、搜索、消息和设置等主要使用路径，定位是个人学习、实验和测试，不是 Coolapk 官方客户端，也不提供独立后端。
...
- 消息页包含动态、关注、粉丝以及 @我、@评论、收到的赞、好友关注、通知等入口。
- 本地保存浏览历史、动态收藏、首页 Tab、搜索历史、最近 @用户、最近表情和用户/话题黑名单。
- 设置支持主题/深色模式、主题色、字体大小、图片质量、外部浏览器、缓存清理、黑名单、设备参数和应用更新检查。

### 账号与应用

- 支持账号密码登录和图形验证码；短信登录相关 API/菜单入口仍需单独核验，不能按完整功能宣传。
- 登录后同步用户信息和通知数量，并在主导航显示消息角标。
...
当前主应用约有 255 个 Kotlin/Java 源文件、418 个 Android 资源文件；本地单元测试当前执行 4 个用例（含网络 Base URL 和日志脱敏测试），instrumentation 仍是模板级测试，尚未形成完整业务回归套件。
